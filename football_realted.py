...
def check_if_point_is_in_ROI(point, ROI):
    # if 
    if ROI[point[1], point[0]] == 255:
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    path_of_image = "./sample_image/radar1.png"
    ...
    pd = PolygonDrawer("new_img" ,img)
    masked, img_required, points = pd.run()

    point_to_check = (500,500)

    print(check_if_point_is_in_ROI(point_to_check, masked))
    cv2.imshow("img_needed",masked)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
